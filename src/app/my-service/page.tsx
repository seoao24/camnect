			...
			<div className='container'>
				<div className={`${isAdd ? '' : 'hidden'}`}>
								...
								<tr>
									...
									<td>
										{concept.map((e) => (
											<button
												key={`concept-${e.id}`}
												className={`rounded-[10px] mx-2 px-4 py-1 ${
													selectedConcept == e.id ? 'bg-[#F07202] text-white' : 'bg-[#DCDCDC]'
												}`}
									...
									<td>
										{additionServices.map((e) => (
											<button
												key={e.id}
												className={`rounded-[10px] mx-2 px-4 py-1 ${
													selectedAdditionervice == e.id
														? 'bg-[#F07202] text-white'
														: 'bg-[#DCDCDC]'
												}`}
					...
					<div className='shadow-md mt-5'>
						<div className='w-full border-[1px] border-[#8E8B8B] flex px-4 py-2'>
							<div className=''>
								<div className={`text-[30px] text-[#F07202] font-bold ${isEditTitle ? 'hidden' : ''}`}>
									{form.type}
								</div>
								<input
									type='text'
									name='type'
									className={`px-5 py-1 border-none outline-none ${isEditTitle ? '' : 'hidden'}`}
									value={form.type}
							...
							<div className='flex items-center text-[#606060] text-[24px]'>
								<div
									className='text-[#B7B7B7] text-[20px] border-l-[1px] border-l-[#B7B7B7] px-5 mx-5 cursor-pointer'
						...
						<div className='flex py-5'>
							<div className='shadow-md rounded-[20px] my-2 mx-2'>
								...
								<label htmlFor='add-service-file'>
									<div
										className='w-[400px] h-[325px] bg-cover bg-center bg-no-repeat'
										style={{
							...
							<div className='w-full px-5'>
								<input
									...
									value={form.name}
									type='text'
									className='border-[#F07202] border-[1px] outline-none px-3 py-1 rounded-[5px] w-full h-[54px]'
								...
								<input
									...
									value={form.price}
									type='number'
									className='border-[#F07202] border-[1px] outline-none px-3 py-1 rounded-[5px] w-full my-5 h-[54px]'
									placeholder='Giá'
									onChange={handleChange}
								/>
								...
								<input
									...
									className='border-[#F07202] border-[1px] outline-none px-3 py-1 rounded-[5px] w-full'
					...
					<div className='flex justify-center my-5'>
						<button
							className='bg-[#EBEBEB] text-[#F07202] rounded-[10px] py-2 px-5 mx-2'
						...
						{!form.id ? (
							<button
								className='bg-[#F07202] rounded-[10px] text-white py-2 px-5 mx-2'
								onClick={addService}
							>
								Đăng dịch vụ
							</button>
				...
				<div className='mt-10 px-5 mb-5'>
					<div className='flex justify-between items-center'>
						...
						<button
							className={`bg-[#F07202] rounded-[5px] text-white px-3 py-2 ${isAdd ? 'hidden' : ''}`}
					...
					<div className='grid md:grid-cols-4 grid-col-1 gap-4'>
						{userServices.map((e) => (
							<div
								className='rounded-[25px] bg-white md:w-full'
								key={e.id}
							...
							>
								...
								<div className='px-1 mt-2'>
									...
									<div className='text-[14px] font-bold text-nowrap text-[#828282] line-through'>
										Giá: {e.oldPrice}
									</div>
