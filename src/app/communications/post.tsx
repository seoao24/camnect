...
interface PostProps {
	...
	totalComment: number;
	likedByUser: boolean;
	onLiked?: (postId: string, isLiked: boolean) => void;
}

export default function Post(props: PostProps) {
	const [isComment, setIsComment] = useState(false);
	const [likeCount, setLikeCount] = useState(props.totalLike);
	const [isLiking, setIsLiking] = useState(false);
	const [hasLiked, setHasLiked] = useState(props.likedByUser);
	// Thêm state để theo dõi hiệu ứng khi nhấn like/dislike
	const [likeAnimation, setLikeAnimation] = useState(false);

	const handleLike = async () => {
		// Nếu đang xử lý thì không làm gì
		if (isLiking) return;
		...
		try {
			if (hasLiked) {
				...
				toast.success('Đã hủy thích bài viết');

				// Cập nhật state ở component cha (nếu có callback)
				if (props.onLiked) {
					// Truyền thêm tham số false để biết là dislike
					props.onLiked(props.id, false);
				}
			} else {
				// Gọi API để like bài viết
				await axiosInstance.post(`/Post/Like?postId=${props.id}`);
				...
				// Cập nhật state ở component cha (nếu có callback)
				if (props.onLiked) {
					// Truyền thêm tham số true để biết là like
					props.onLiked(props.id, true);
				}
			}
		} catch (error) {
			console.error('Error toggling like status:', error);
			toast.error(hasLiked ? 'Không thể hủy thích bài viết' : 'Không thể thích bài viết');
		} finally {
			setIsLiking(false);
		}
	};
	...
	return (
		<div className='card shadow-lg py-5 md:px-10 px-5 my-5 rounded-[20px] w-full'>
			...
			<div className='py-3 text-[14px]'>{props.description}</div>
			<div
				className={`grid ${
			...
			<div className='flex justify-between mt-10'>
				<button
					className={`flex items-center relative ${likeAnimation ? 'animate-pulse' : ''}`}
					onClick={handleLike}
					disabled={isLiking}
					title={hasLiked ? 'Hủy thích bài viết' : 'Thích bài viết'}
				>
					{/* Indicator thể hiện trạng thái like */}
					<div className='absolute -top-5 left-0 right-0 text-center text-xs font-medium'>
						{hasLiked ? (
							<span className='text-[#F07202]'>Đã thích</span>
						) : (
							<span className='text-transparent'>Chưa thích</span>
						)}
					</div>

					<div
						className={`relative ${
							likeAnimation ? 'transform scale-110 transition-transform duration-200' : ''
						}`}
					>
						<svg
							height='30px'
							...
							<g id='SVGRepo_iconCarrier'>
								...
								<style
									...
									dangerouslySetInnerHTML={{
										__html: ` .st0{fill:${hasLiked ? '#F07202' : '#666967'};} `,
									}}
								/>{' '}
								<g>
									{' '}
						...
						{/* Hiển thị icon nhỏ phía dưới góc để chỉ rõ hành động */}
						{hasLiked && (
							<div className='absolute -bottom-1 -right-1 bg-white rounded-full border border-[#F07202] w-4 h-4 flex items-center justify-center'>
								<span className='text-[#F07202] text-[8px]'>✓</span>
							</div>
						)}
					</div>

					<div className={`ml-2 text-[20px] ${hasLiked ? 'text-[#F07202]' : 'text-[#666967]'}`}>
						{likeCount} like
					</div>

					{/* Loading indicator khi đang xử lý */}
					{isLiking && (
						<div className='ml-2 w-4 h-4 border-2 border-t-transparent border-[#F07202] rounded-full animate-spin'></div>
					)}
				</button>
					...
					>
						...
						<g id='SVGRepo_iconCarrier'>
							<defs>
								<style
									dangerouslySetInnerHTML={{
										__html: '.cls-1{fill:none;stroke:#666967;stroke-miterlimit:10;stroke-width:1.91px;}',
									}}
								/>
							</defs>
							<path
				...
				<button className='flex items-center'>
					<div
						className='bg-contain bg-center bg-no-repeat w-[30px] h-[30px]'
						style={{
