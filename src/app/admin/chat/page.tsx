		...
		<div className='flex h-screen bg-gray-100'>
			...
			<div className='w-1/4 bg-white shadow p-4 border-r'>
				<input
					...
					className='w-full p-2 border rounded-lg mb-4 outline-none'
					onChange={(e) => {
				...
				<div className='space-y-4 overflow-y-auto scrollbar-hide max-h-[600px]'>
					{users.map((e, index) => (
						...
						>
							<div
								className='w-[40px] h-[40px] rounded-full bg-cover bg-center bg-no-repeat bg-[#D9D9D9] text-center leading-[40px] font-semibold text-[#F07202]'
								style={{
									backgroundImage: `url('https://api.tapta.online/${e.avatarUrl}')`,
								}}
							>
				...
				<div className='flex items-center justify-between p-4 bg-white shadow'>
					<div className='flex items-center space-x-3'>
						<div
							className='w-[40px] h-[40px] rounded-full bg-cover bg-center bg-no-repeat bg-[#D9D9D9] text-center leading-[40px] font-semibold text-[#F07202]'
							style={{
								backgroundImage: `url('https://api.tapta.online/${selectedUser?.avatarUrl}')`,
							}}
						>
				...
				<div className='flex-1 overflow-y-auto p-4 bg-gray-50'>
					{messages.map((msg, index) => (
						<div key={msg.Id + '-message-' + index} className='mb-4 flex items-end space-x-3'>
							<div
								className='w-[30px] h-[30px] rounded-full bg-cover bg-center bg-no-repeat bg-[#D9D9D9] text-center leading-[40px] font-semibold text-[#F07202]'
								style={{
									backgroundImage: `url('https://api.tapta.online/${selectedUser?.avatarUrl}')`,
								}}
							>
							...
							<div>
								<p className='text-sm font-semibold'>
									...
									<span className='text-xs text-gray-500'>{(msg.Time ?? '').toString()}</span>
								</p>
