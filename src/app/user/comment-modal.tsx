					...
					{comments.map((e) => (
						<div key={e.commentId} className='py-2'>
							<div className='flex items-center'>
								<div
									className='bg-cover bg-center bg-no-repeat w-[30px] h-[30px] rounded-full bg-[#D9D9D9]'
									style={{
										backgroundImage: `url('https://api.tapta.online/${e.userAvatarUrl}')`,
									}}
								></div>
								<div className='text-[14px] font-semibold text-black ml-2'>{e.createdBy}</div>
							</div>
							<div className='text-[16px] ml-[40px]'>{e.content}</div>
						</div>
					))}
				</div>
